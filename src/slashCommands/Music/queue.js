...
const embed = new MessageEmbed()
  ...
  .setDescription(`Now playing [${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${pms(player.position)} / ${pms(player.queue.current.duration)} ]\` • [${getRequesterName(player.queue.current.requester)}]`);
...
embed3.setTitle(`${interaction.guild.name} Server Queue`)
  .setColor(client.embedColor)
