...
module.exports = {
   ...
   execute: async (message, args, client, prefix) => {
            ...
            if(player.queue.length === "0" || !player.queue.length) {
                ...
                .setDescription(`Sedang dimainkan [${player.queue.current.title}](${player.queue.current.uri}) • \`[ ${pms(player.position)} / ${pms(player.queue.current.duration)} ]\` • [${player.queue.current.requester}]`)
                ...
                if(player.queue.size < 11) {
                    ...
                    .setTimestamp()
                    .setFooter({ text: `Page ${page + 1}/${pages.length}`, iconURL: message.author.displayAvatarURL({ dynamic: true, format: 'png' }) })
                    .setThumbnail(player.queue.current.thumbnail)
                    .setTitle(`${message.guild.name} Antre`)
                    ...
                    const collector = message.channel.createMessageComponentCollector({
                        filter: (b) => {
                            if(b.user.id === message.author.id) return true;
                                ...
                                b.reply({
                                    ephemeral: true,
                                    content: `Hanya **${message.author.tag}** dapat menggunakan tombol ini, jika mau maka Anda harus menjalankan perintah lagi.`
                                });
                                return false;
                            };
                        },
