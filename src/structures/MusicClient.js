...
class MusicBot extends Client {
    ...
    */
        ...
        mongoose.connection.on('connected', () => {
              this.logger.log('[MongoAtlas] Database Terkoneksi', "ready");
              });
        mongoose.connection.on('err', (err) => {
                  console.log(`Mongoose kesalahan koneksi: \n ${err.stack}`, "error");
              });
 ...
 */
  ...
  readdirSync("./src/slashCommands/").forEach((dir) => {
        ...
        for (const file of slashCommandFile) {
            ...
            this.logger.log(`Perintah SlashCommands Klien (/) Dimuat: ${slashCommand.name}`, "cmd");
            data.push(slashCommand);
        }
     });
	  this.on("ready", async () => {
        await this.application.commands.set(data).then(() => this.logger.log(`Aplikasi Klien (/) Terdaftar.`, "cmd")).catch((e) => console.log(e));
    });
	 }
		 connect() {
