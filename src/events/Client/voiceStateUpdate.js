  ...
  switch (stateChange.type) {
    case "JOIN":
      if (stateChange.members.size === 1 && player.paused) {
        let emb = new MessageEmbed()
          ...
          .setDescription(
            `Melanjutkan pemutaran karena Anda semua meninggalkanku dengan musik untuk diputar sendirian`
          );
    ...
    case "LEAVE":
      if (stateChange.members.size === 0 && !player.paused && player.playing) {
        ...
        let emb = new MessageEmbed()
          .setAuthor({ name:`Dijeda!`})
          .setColor(client.embedColor)
          .setDescription(`Pemain telah dijeda karena semua orang keluar`);
        await client.channels.cache.get(player.textChannel).send({embeds: [emb]});
      }
      break;
  }
